import { ActionRowBuilder, ColorResolvable, ButtonBuilder, EmbedBuilder, ButtonStyle, GuildMember, TextChannel, Events } from 'discord.js';
...
export const execute = async (member: GuildMember) => {
  try {
    ...
    const memberJoinLoggerEmbed = new EmbedBuilder()
      .setDescription(
        `Member Joined (${member.user.id}) - <@${member.user.id}>${member.user.bot ? ' (Bot)' : ''}${member.user.createdTimestamp > sevenDaysAgo.getTime() ? ' (New Account)' : ''}`
      )
    ...
    const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
      new ButtonBuilder().setLabel('Kick').setCustomId(`LOGGING_KICK_USER_${member.user.id}`).setStyle(ButtonStyle.Danger),
      new ButtonBuilder().setLabel('Ban').setCustomId(`LOGGING_BAN_USER_${member.user.id}`).setStyle(ButtonStyle.Danger)
    );
    await loggingChannel.send({
      content: `User Joined - ${member.user.id}${member.user.bot ? ' (Bot)' : ''}${member.user.createdTimestamp > sevenDaysAgo.getTime() ? ' (New Account) @here' : ''}`,
