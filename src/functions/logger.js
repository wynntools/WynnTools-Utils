...
function warnMessage(message) {
  return console.log(`${cli.bgYellow.black(`[${getCurrentTime()}] Warning >`)} ${cli.yellow(message)}`);
}
...
function cacheMessage(type, message) {
  return console.log(`${cli.bgBlueBright.black(`[${getCurrentTime()}] ${type} Cache >`)} ${cli.blueBright(message)}`);
}

function eventMessage(event, message) {
  return console.log(`${cli.bgBlueBright.black(`[${getCurrentTime()}] ${event} Event >`)} ${cli.blueBright(message)}`);
}
